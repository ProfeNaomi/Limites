...
import { contextExercises } from "../data/contextLimits";
import { CheckCircle2, LineChart } from "lucide-react";

export function Applications() {
  const [openExercise, setOpenExercise] = useState<number>(1);

  const activeExe = contextExercises.find((e) => e.id === openExercise);
      ...
      <div className="border-b border-slate-200 pb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold leading-7 text-slate-900 sm:truncate sm:text-3xl sm:tracking-tight flex items-center gap-2">
            ...
            Límites en Contextos Reales
          </h2>
          <p className="mt-2 text-slate-500 text-lg">
            Descubre cómo se aplican los límites para resolver problemas del mundo real.
          </p>
        </div>
        <div className="mt-4 sm:mt-0 flex items-center bg-indigo-50 text-indigo-700 px-4 py-2 rounded-full text-sm font-semibold border border-indigo-100 flex-shrink-0">
          ...
          {contextExercises.length} Casos Disponibles
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {contextExercises.map((exe) => {
          ...
          return (
            ...
            >
              <div className={`text-xs font-bold uppercase tracking-wider mb-2 ${isOpen ? "text-indigo-600" : "text-slate-400"}`}>
                Caso {exe.id}
              </div>
              <div className={`text-sm font-semibold text-center mb-3 ${isOpen ? "text-indigo-900" : "text-slate-700"}`}>
                {exe.title}
              </div>
        ...
        <div className="mt-10 bg-white rounded-3xl border border-indigo-100 shadow-xl overflow-hidden animate-in slide-in-from-bottom-8 duration-500">
          <div className="bg-gradient-to-r from-indigo-50 to-white px-6 py-8 sm:px-10 border-b border-indigo-50">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="w-full sm:w-auto overflow-hidden">
                <h3 className="text-sm font-bold text-indigo-600 uppercase tracking-widest mb-2">Contexto del Problema</h3>
                <h4 className="text-xl font-bold text-slate-900 mb-4">{activeExe.title}</h4>
                <p className="text-slate-700 text-lg mb-6 leading-relaxed max-w-4xl">
                  {renderDescription(activeExe.problem)}
                </p>
                
                <h3 className="text-sm font-bold text-indigo-600 uppercase tracking-widest mb-4">Límite a Calcular</h3>
          ...
          <div className="px-6 py-8 sm:px-10 bg-slate-50/50">
            <h4 className="text-base font-bold text-slate-800 tracking-wide mb-8 flex items-center">
              <span className="bg-indigo-100 px-4 py-1.5 rounded-full text-indigo-700 text-sm">Resolución Paso a Paso</span>
            </h4>
